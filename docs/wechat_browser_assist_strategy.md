...
> 当前项目不内置完整 Browser Agent，也不默认启用浏览器自动化。API 无法覆盖的后台事项优先通过 `docs/external_agent_task_package_design.md` 中的外部 Agent 任务包记录为人工检查清单，交给 Hermes、Cursor Agent、Playwright MCP、Browser Use 或用户手动核对。
...
| 核对缺口字段 | browser_assist | 封面裁剪、合集、通知、后台发布/定时发布等人工事项 |
| 截图 | 用户 | 本地路径，不含凭据 |
| 人工确认 | 用户 | 默认仅保存，不自动发布 |
| proof 回填 | 用户 + 本地 | Round 19 接入最小记录 |

人工确认点（checkpoint）：`login` → `draft_located` → `fields_reviewed` → `manual_backend_action_by_user` → `proof_backfill`。
