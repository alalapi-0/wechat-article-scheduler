...
> **产品定位（Phase 10 起的权威表述）**：本项目是「本地作品 → 公众号」的**批量发布工作台**，不是审核平台。用户从网页**批量上传文章与封面**，上传的内容即视为想发布的作品，**没有"审核"步骤**。真实发布的安全保障 = 默认演练(mock) + 显式开关(`WECHAT_ENABLE_PUBLISH`) + 执行到点前的二次确认 + 发布前预检清单。历史轮次（Round 0–42）中出现的"审核/review_status"叙事已在 Round 43 移除，仅保留人话文案、桌面信息架构与发布可靠性等成果。
...
- 当前已实现：CLI scan/plan/run-once、mock/real adapter 基础能力、SQLite 审计、桌面优先 Web 工作台、**网页批量上传作品与封面**、每篇作品独立封面、发布前确认与预检、治理 gate。
...
- 暂不做：React/Vue 控制台、商业化多用户后台、内容审核/审批流、网页登录公众号后台、cookie 保存、默认真实发布。
