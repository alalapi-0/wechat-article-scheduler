...
Promise.all([
...
]).then(async ([a,b,c,d,e,f,g,h,i,j,k,l,m,n,o,p])=>{
  ...
  document.getElementById('adapter-registry').textContent = JSON.stringify(await h.json(), null, 2);
  document.getElementById('manifest-dry-run').textContent = JSON.stringify(await i.json(), null, 2);
  document.getElementById('local-blog-static').textContent = JSON.stringify(await j.json(), null, 2);
  document.getElementById('local-blog-wp').textContent = JSON.stringify(await k.json(), null, 2);
  document.getElementById('webhook-plan').textContent = JSON.stringify(await l.json(), null, 2);
  document.getElementById('video-package').textContent = JSON.stringify(await m.json(), null, 2);
  document.getElementById('wechat-chain').textContent = JSON.stringify(await n.json(), null, 2);
  document.getElementById('waiting').textContent = JSON.stringify(await o.json(), null, 2);
  document.getElementById('outbox').textContent = JSON.stringify(await p.json(), null, 2);
});
</script></body></html>"""
